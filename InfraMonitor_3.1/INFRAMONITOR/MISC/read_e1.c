...
#define USAGE "usage: Y = READ_E1 ( DATAFILE, BYTEOFFSET, NUM )\n"
...
#define	DATAFILE	prhs[0]
#define	BYTEOFFSET	prhs[1]
#define	NUM		prhs[2]
...
void mexFunction ( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
  ...
  */
  ...
  /*************************************************
  ** Get data file name as a string.
  */
  mxGetString ( DATAFILE, datafile, 1023 );
  ...
  */
  if ( nrhs < 2 )
    ...
    byteoffset = ( int )( * mxGetPr ( BYTEOFFSET ) );
  ...
  ** Open data file.
  */
  data_fp = fopen ( datafile, "rb" );
  if ( data_fp == NULL )
  {
    mexPrintf ( "Could not open data file: '%s'\n", datafile );
  ...
  */
  for ( rec_num=0, total_samp=0 ; rec_num<MAX_REC ; ++rec_num )
  {
    ...
    ** Read header.
    */
    num_read = fread ( xint, sizeof(int), 2, data_fp );
    if ( num_read != 2 )
      break;
    ...
    */
    comp_length = comp_size/4 - 2;
    comp_data = mxCalloc ( comp_length, sizeof(int) );

    /*************************************************
    ** Read entire data record.
    */
    num_read = fread ( comp_data, sizeof(int), comp_length, data_fp );
    ...
    */
    uncomp_data[rec_num] = mxCalloc ( num_samp[rec_num], sizeof(int) );
    ...
    */
    ...
    {
      mexPrintf ( "Error decompressing \"%s\":\n", datafile );
      mexPrintf ( "check value does not match last value.\n" );
      break;
    }
